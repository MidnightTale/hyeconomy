...
public class DepositCommand {
    public static void execute(Player player, String[] args, MiniMessage mm) {
        if (args.length != 1) {
            player.sendMessage((Component) MessageUtil.getMessage("depositUsage"));
            return;
        }
        ...
        try {
            ...
            if (amount <= 0) {
                player.sendMessage((Component) MessageUtil.getMessage("depositAmountPositive"));
                return;
            }
            ...
            if (diamondsInInventory < amount) {
                player.sendMessage((Component) MessageUtil.getMessage("depositNotEnoughDiamonds"));
                return;
            }
            ...
            player.sendMessage((Component) MessageUtil.getMessage("depositSuccess", "%amount%", String.valueOf(amount), "%new_balance%", String.valueOf(newBalance)));
